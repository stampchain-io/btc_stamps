...
def create_check_hashes(db, block_index, valid_stamps_in_block: list[ValidStamp], processed_src20_in_block, txhash_list,
    ...
    new_txlist_hash, found_txlist_hash = check.consensus_hash(db, block_index, 'txlist_hash', previous_txlist_hash, txlist_content)

    ledger_content = str(processed_src20_in_block)
    new_ledger_hash, found_ledger_hash = check.consensus_hash(db, block_index, 'ledger_hash', previous_ledger_hash, ledger_content)

    messages_content = str(txhash_list)
    new_messages_hash, found_messages_hash = check.consensus_hash(db, block_index, 'messages_hash', previous_messages_hash, messages_content)
