...
BLOCK_TEST_DATA = {
    865002: BLOCK_865002_TX_HASHES,
    867315: BLOCK_867315_TX_HASHES
}
...
def test_expected_transactions_included(tx_data: Dict[str, str], expected_tx_hashes: List[str], block_index: int) -> Tuple[int, int, Set[str]]:
    ...
    for tx_hash, tx_hex in tx_data.items():
        ...
        if should_include:
            included_count += 1
            logger.info(f"Transaction {tx_hash} INCLUDED: pattern={has_valid_pattern}, data={has_valid_data}, keyburn={keyburn}")
            ...
            logger.debug(f"Transaction {tx_hash} not included: pattern={has_valid_pattern}, data={has_valid_data}, keyburn={keyburn}")
...
def test_src20_processing_simplified(tx_data: Dict[str, str], expected_tx_hashes: List[str], block_index: int) -> Tuple[int, int]:
...
def parse_args():
    ...
    parser.add_argument('--block', type=int, default=865002, 
                        help='Block number to test (default: 865002, also available: 867315)')
