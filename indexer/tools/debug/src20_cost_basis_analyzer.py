...
class SRC20CostBasisAnalyzer:
        ...
        if not result or result["count"] == 0:
            ...
            if not result or result["count"] == 0:
                ...
                actual_tick = self.fetch_one(
                    "SELECT DISTINCT tick FROM SRC20Valid WHERE UPPER(tick) = UPPER(%s) LIMIT 1", (tick,)
                )
                if actual_tick:
    ...
    def analyze(self, tick: str, output_format: str = "json", generate_chart: bool = False):
        ...
        # Prepare output data
        output_data = {"token_info": token_info, "statistics": stats, "mint_transactions": mint_transactions}
    ...
    def list_tokens(self):
        ...
        for row in results:
            ...
            print(
                f"{row['tick']:<10} {row['mint_count']:<10} {total_minted:<20.4f} {row['first_mint_block']:<12} {row['last_mint_block']:<12} {row['unique_minters']:<10}"
            )
