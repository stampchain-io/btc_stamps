...
class TestParser(unittest.TestCase):
    ...
    def test_parse_block(self, mock_fast_parser):
        ...
        mock_parser_instance.parse_block.return_value = (tx_hash_list, raw_transactions, timestamp, prev_block_hash, bits)
...
class TestEnhancedCTransaction(unittest.TestCase):
    ...
    def test_enhanced_ctransaction_creation(self):
        """Test creating an EnhancedCTransaction."""
        from bitcoin.core import CTransaction

        from index_core.parser import EnhancedCTransaction

        # Create a real CTransaction instance
        ctx = CTransaction([], [])  # Empty transaction

        # Test successful creation
        enhanced = EnhancedCTransaction(ctx, txid="test_txid", custom_attr="value")
        self.assertEqual(enhanced._ctx, ctx)
    ...
    def test_enhanced_ctransaction_attribute_access(self):
        """Test attribute access in EnhancedCTransaction."""
        from bitcoin.core import CTransaction

        from index_core.parser import EnhancedCTransaction

        # Create a real CTransaction with some attributes
        ctx = CTransaction([], [], nVersion=1)
        ...
        self.assertEqual(enhanced.nVersion, 1)
