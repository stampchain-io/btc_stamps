...
from index_core.src20 import Src20Validator, convert_to_utf8_string, matches_any_pattern
...
class TestUnicodeEmojiHandling(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.emoji_tokens = ["brun🔥", "btc🚀", "lumi💫", "👽", "🥷", "🧧"]

        self.mixed_case_tokens = [("BALD", "bald"), ("GME", "gme"), ("JOKR", "jokr"), ("KING", "king")]
    ...
    def test_src20_validator_tick_processing(self):
        ...
        for original_token, expected_escaped in expected_conversions.items():
            src20_dict = {"tick": original_token, "p": "src-20", "op": "deploy"}
    ...
    def test_case_insensitive_matching_scenarios(self):
        ...
        for lower_token, upper_token in test_cases:
            ...
            print(
                f"Old way: {old_normalized_lower} == {old_normalized_upper} -> {old_normalized_lower == old_normalized_upper}"
            )
            print(
                f"New way: {new_normalized_lower} == {new_normalized_upper} -> {new_normalized_lower == new_normalized_upper}"
            )
