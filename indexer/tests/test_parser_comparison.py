...
def test_parser_comparison_deploy(dummy_db, deploy_src20_dict):
    ...
    processed_list_rust = []
    is_valid_rust, result_rust = parse_rust_src20(dummy_db, deploy_src20_dict, processed_list_rust)

    # Compare key fields: validation flag, tick (normalized), tick_hash, and op
    assert is_valid_py == is_valid_rust, "Validation flags should match between Python and Rust parser detections."
    assert result_py.get("tick") == result_rust.get("tick"), "Tick values should match (both normalized)."
    assert result_py.get("tick_hash") == result_rust.get("tick_hash"), "Tick hashes should match between parsers."
    assert result_py.get("op") == result_rust.get("op"), "Operation types should be identical."

    # Optionally, compare numeric fields if present
    if result_py.get("max") and result_rust.get("max"):
        assert Decimal(result_py.get("max")) == Decimal(result_rust.get("max")), "Max values should be equivalent."
