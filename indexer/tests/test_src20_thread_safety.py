...
def sample_transactions():
    ...
    for i in range(20):
        transactions.append(
            {"txid": f"txid{i}", "blockheight": 100000 + i, "tick": f"TEST{i}", "amount": "100", "data": f"Sample data {i}"}
        )
    return transactions
...
def test_thread_safety_basic(sample_transactions, thread_lock):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(processor.process_transaction, tx) for tx in sample_transactions]
...
def test_thread_safety_duplicate_ticks(thread_lock):
    ...
    for i in range(10):
        ...
        tick = f"TEST{i}"
        transactions.append({"txid": f"txid{i}_1", "blockheight": 100000 + i, "tick": tick, "amount": "100"})
        transactions.append({"txid": f"txid{i}_2", "blockheight": 100001 + i, "tick": tick, "amount": "100"})
    ...
    # Process transactions concurrently
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        futures = [executor.submit(processor.process_transaction, tx) for tx in transactions]
...
def test_lock_importance():
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(increment_counter_without_lock, iterations // num_threads) for _ in range(num_threads)]
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(increment_counter_with_lock, iterations // num_threads) for _ in range(num_threads)]
        concurrent.futures.wait(futures)
...
def test_src20_with_real_implementation():
    ...
    with mock.patch.dict(
        "sys.modules",
        {
            ...
            "index_core.config": mock.MagicMock(
                TICK_PATTERN_SET=set("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
            ),
        },
    ):
