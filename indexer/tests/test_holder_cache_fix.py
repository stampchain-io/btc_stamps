...
def mock_database_manager():
    """Mock the database manager to avoid connection requirements."""
    with patch('index_core.database_manager.DatabaseManager') as mock_db_manager:
...
def mock_counterparty_api():
    ...
    def mock_api_response(endpoint, params):
        if "dispensers" in endpoint:
            return {"result": [
                {"status": 0, "satoshirate": "1000", "give_quantity": "100"}
            ]}
        elif "dispenses" in endpoint:
            return {"result": [
                {"block_time": 1700000000, "satoshirate": "1500", "dispense_quantity": "10"}
            ]}
...
class TestHolderCacheFix:
    ...
    def test_holder_cache_data_generation(self, mock_database_manager, mock_counterparty_api):
        ...
        assert all(isinstance(holder["quantity"], float) and holder["quantity"] > 0
    ...
    def test_holder_cache_data_extraction(self, mock_database_manager, mock_counterparty_api):
    ...
    def test_holder_cache_with_invalid_balances(self, mock_database_manager, mock_invalid_balances_api):
    ...
    def test_populate_holder_cache_database_logic(self):
        ...
        delete_calls = [call for call in mock_cursor.execute.call_args_list
                       if call[0] and 'DELETE' in call[0][0]]
